...
async function migrateStatusValues(dryRun = true) {
  ...
  if (dryRun) {
    ...
    console.log(
      "⚠️ MODO EJECUCIÓN - Se aplicarán los cambios en la base de datos\n"
    );
  }
  ...
  for (const evento of eventosParaMigrar) {
    ...
    console.log(
      `  📌 Evento "${evento.nombre}" | ${evento.status} -> ${nuevoStatus}`
    );
    ...
    if (!dryRun) {
      try {
        ...
        console.error(
          `    ❌ Error migrando evento ${evento.id}:`,
          error.message
        );
  ...
  for (const cotizacion of cotizacionesParaMigrar) {
    ...
    console.log(
      `  📌 Cotización "${cotizacion.nombre}" | ${cotizacion.status} -> ${nuevoStatus}`
    );
    ...
    if (!dryRun) {
      try {
        ...
        console.error(
          `    ❌ Error migrando cotización ${cotizacion.id}:`,
          error.message
        );
  ...
  for (const pago of pagosParaMigrar) {
    ...
    console.log(
      `  📌 Pago "${pago.concepto}" | ${pago.status} -> ${nuevoStatus}`
    );
  ...
  for (const item of agendaParaMigrar) {
    ...
    console.log(
      `  📌 Agenda "${item.concepto}" | ${item.status} -> ${nuevoStatus}`
    );
  ...
  for (const cliente of clientesParaMigrar) {
    ...
    console.log(
      `  📌 Cliente "${cliente.nombre}" | ${cliente.status} -> ${nuevoStatus}`
    );
    ...
    if (!dryRun) {
      try {
        ...
        console.error(
          `    ❌ Error migrando cliente ${cliente.id}:`,
          error.message
        );
  ...
  const totalMigrados = Object.values(migrationResults).reduce(
    (sum, r) => sum + r.migrados,
    0
  );
  const totalErrores = Object.values(migrationResults).reduce(
    (sum, r) => sum + r.errores,
    0
  );
  ...
  if (dryRun) {
    ...
    console.log(
      "💡 Ejecuta el script de validación para verificar los resultados."
    );
  }
