...
async function validateStatusConsistency() {
  ...
  const validCotizacionStatus = Object.values(
    STATUS_CONSTANTS.COTIZACION_STATUS
  );
  ...
  Object.entries(validacionResults).forEach(([tabla, result]) => {
    ...
    const porcentaje =
      total > 0 ? ((result.validos / total) * 100).toFixed(1) : "0.0";
    ...
    if (result.invalidos > 0) {
      ...
      result.detalles.slice(0, 5).forEach((detalle) => {
        console.log(
          `    - ID: ${detalle.id} | Nombre: ${detalle.nombre || detalle.concepto} | Status: "${detalle.status}"`
        );
      });
  ...
  const totalValidos = Object.values(validacionResults).reduce(
    (sum, r) => sum + r.validos,
    0
  );
  const totalInvalidos = Object.values(validacionResults).reduce(
    (sum, r) => sum + r.invalidos,
    0
  );
  const total = totalValidos + totalInvalidos;
  const porcentajeGeneral =
    total > 0 ? ((totalValidos / total) * 100).toFixed(1) : "0.0";

  console.log("\n" + "=".repeat(80));
  console.log(
    `🎯 RESULTADO GENERAL: ${totalValidos}/${total} registros válidos (${porcentajeGeneral}%)`
  );
  ...
  if (totalInvalidos > 0) {
    ...
    console.log(
      "💡 Ejecuta el script de migración para homologar estos valores."
    );
  } else {
