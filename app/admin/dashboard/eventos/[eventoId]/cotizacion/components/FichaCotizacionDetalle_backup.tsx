...
const getStatusBadgeStyles = (status: string | undefined) => {
    if (!status) return 'bg-zinc-800 text-zinc-300 border-zinc-600'
...
export default function FichaCotizacionDetalle({ cotizacion, onEliminarCotizacion, eventoId }: Props) {
    ...
    return (
            return (
        <div className={`relative bg-zinc-900/40 border border-zinc-700/50 rounded-xl p-5 space-y-4 transition-all hover:border-zinc-600/50 ${archivada ? 'bg-amber-900/20 border-amber-600/60' : ''}`}>
            ...
            {/* Línea 1: Nombre cotización | menú modal */}
            <div className={`flex items-center justify-between gap-4 ${archivada ? 'opacity-30' : ''}`}>
                <div className="flex items-center gap-3 flex-1 min-w-0">
                    <button
                        onClick={() => router.push(`/admin/dashboard/eventos/${eventoId}/cotizacion/${cotizacion.id}`)}
                        className="text-zinc-400 hover:text-zinc-200 transition-colors p-1.5 hover:bg-zinc-800/60 rounded-md"
                    ...
                    <button
                        onClick={() => router.push(`/admin/dashboard/eventos/${eventoId}/cotizacion/${cotizacion.id}`)}
                        className="font-semibold text-zinc-100 hover:text-white hover:underline text-left break-words text-lg flex-1 min-w-0"
            ...
            {/* Línea 2: Descripción (si la hay) */}
            {cotizacion.descripcion && (
                <div className={`text-sm text-zinc-300 break-words leading-relaxed ${archivada ? 'opacity-30' : ''}`}>
                    {cotizacion.descripcion}
                </div>
            )}
            {!cotizacion.descripcion && cotizacion.servicios?.length && (
                <div className={`text-sm text-zinc-400 italic ${archivada ? 'opacity-30' : ''}`}>
                    {cotizacion.servicios.length} servicios incluidos
                </div>
            )}

            {/* Línea 3: Status */}
            <div className={`flex items-center ${archivada ? 'opacity-30' : ''}`}>
                <span className={`text-xs px-3 py-1.5 rounded-full border font-medium ${getStatusBadgeStyles(cotizacion.status)}`}>
                    {getStatusDisplayName(cotizacion.status)}
                </span>
            </div>

            {/* Línea 4: Vistas */}
            <div className={`flex items-center gap-2 text-zinc-400 text-sm ${archivada ? 'opacity-30' : ''}`}>
                <Eye className="w-4 h-4" />
                <span className="font-medium">{cotizacion.visitas} vistas</span>
            </div>

            {/* Línea 5: Precio */}
            <div className={`text-xl font-bold text-emerald-400 ${archivada ? 'opacity-30' : ''}`}>
                {cotizacion.precio.toLocaleString('es-MX', { style: 'currency', currency: 'MXN' })}
            </div>

            {/* Línea 6: Creada el */}
            <div className={`text-xs text-zinc-400 flex items-center gap-2 ${archivada ? 'opacity-30' : ''}`}>
            ...
            {/* Línea 7: Modificada el (si aplica) */}
            {cotizacion.updatedAt && cotizacion.updatedAt !== cotizacion.createdAt && (
                <div className={`text-xs text-zinc-400 flex items-center gap-2 ${archivada ? 'opacity-30' : ''}`}>
                    <Check className="w-3 h-3" />
                    <span>
                        Modificada el {new Date(cotizacion.updatedAt).toLocaleString('es-MX', {
                            year: 'numeric',
                            month: 'long',
                            day: 'numeric',
                            hour: 'numeric',
            ...
            {cotizacion.status === COTIZACION_STATUS.AUTORIZADO && (
                <div className={`p-4 bg-blue-900/40 border border-blue-600/50 rounded-lg backdrop-blur-sm ${archivada ? 'opacity-30' : ''}`}>
            ...
            {cotizacion.status === COTIZACION_STATUS.APROBADA && (
                <div className={`p-4 bg-emerald-900/40 border border-emerald-600/50 rounded-lg backdrop-blur-sm ${archivada ? 'opacity-30' : ''}`}>
                    <div className="flex items-center justify-between">
                        ...
                        <button
                            ...
                            className="text-emerald-200 hover:text-emerald-100 text-xs px-3 py-1.5 bg-emerald-800/60 hover:bg-emerald-800/80 rounded-md transition-colors font-medium"
                        >
                            Ir a seguimiento
                        </button>
                    </div>
            ...
            <div className="space-y-3">
                ...
                {/* Línea 3: Status */}
                <div className={`flex items-center gap-3 ${archivada ? 'opacity-30' : ''}`}>
                    <span className={`text-xs px-3 py-1.5 rounded-full border font-medium ${getStatusBadgeStyles(cotizacion.status)}`}>
                        {getStatusDisplayName(cotizacion.status)}
                    </span>
                </div>
            ...
            <div className={`bg-zinc-800/30 rounded-lg p-3 border border-zinc-700/30 ${archivada ? 'opacity-30' : ''}`}>
