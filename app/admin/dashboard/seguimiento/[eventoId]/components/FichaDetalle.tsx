...
import { obtenerEventoSeguimiento, actualizarEtapaEvento } from '@/app/admin/_lib/actions/evento/evento.actions'
...
export default function FichaDetalle({ eventoId }: Props) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Obtener etapas
                const etapas = await obtenerEventoEtapas()
                setEtapas(etapas);

                // Obtener datos del evento usando la nueva estructura
                const result = await obtenerEventoSeguimiento(eventoId)

                if (result.success && result.data) {
                    const { evento, cliente, cotizacion, tipoEvento, cotizacionServicio, categorias, usuarios } = result.data

                    setEvento(evento ?? null)
                    setEtapaActual(evento?.eventoEtapaId ?? null)
                    setCliente(cliente ?? null)
                    setCotizacion(cotizacion ?? null)
                    setTipoEvento(tipoEvento ?? null)
                    setServicioCategoria(categorias ?? null)
                    setUsers(usuarios ?? null)

                    // Procesar servicios de cotización
                    setCotizacionServicio(
                        cotizacionServicio
                            ? cotizacionServicio
                                .filter(servicio => servicio.servicioId !== null)
                                .map(servicio => ({
                                    ...servicio,
                                    servicioId: servicio.servicioId as string,
                    ...
                    // Obtener condición comercial si existe
                    if (cotizacion?.condicionesComercialesId) {
                        const condicionComercial = await obtenerCondicionComercial(cotizacion.condicionesComercialesId)
                        const condicion = `${condicionComercial?.nombre} - ${condicionComercial?.descripcion}`
                        setCondicionComercial(condicion)
                    }
                } else {
                    console.error('Error obteniendo datos del evento:', result.error)
                }
    ...
    const handleActualizarEtapa = useCallback(async (etapaId: string) => {
        try {
            ...
            const result = await actualizarEtapaEvento({
                eventoId,
            ...
            if (result.success) {
                ...
                setActualizandoEtapa(false);
                console.error('Error actualizando etapa:', result.error);
            }
