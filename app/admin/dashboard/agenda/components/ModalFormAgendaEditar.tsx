...
import { obtenerAgendaPorEvento, actualizarAgendaEvento } from '@/app/admin/_lib/agenda.actions'
...
export default function ModalFormAgendaNuevo({ agendaId, agendaTipos, onClose, onSubmit }: Props) {
    ...
    }, [concepto, fecha, agendaTipo])

    //! CARGAR DATOS INICIALES
    useEffect(() => {
        setCargandoAgenda(true);
        obtenerAgendaPorEvento(agendaId).then((response) => {
            const agenda = response;
            if (agenda) {
                setConcepto(agenda.concepto);
                setDescripcion(agenda.descripcion);
                setGoogleMapsUrl(agenda.googleMapsUrl);
                setDireccion(agenda.direccion);
                setFecha(agenda.fecha);
                setHora(agenda.hora);
                setAgendaTipo(agenda.agendaTipo);
            }
    ...
    const handleActualizar = useCallback(() => {
        ...
        setGuardando(true);
        actualizarAgendaEvento(agenda).then((response) => {
            if (response.success) {
                onSubmit();
            }
        }).catch((error) => {
    ...
    }, [concepto, descripcion, googleMapsUrl, direccion, fecha, hora, agendaTipo, agendaId, onSubmit, validarFormulario])
                        ...
                        <div>
                            ...
                            <div className='mb-5'>
                                ...
                                <input
                                    ...
                                    className='shadow appearance-none border border-yellow-800 rounded w-full py-2 px-3 text-zinc-200 bg-zinc-900 leading-tight focus:outline-none focus:shadow-outline'
                                />
