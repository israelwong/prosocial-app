...
export default function ServicioForm({ servicio, categorias, configuracion }: Props) {
    ...
    const {
        ...
        const utilidadPorcentaje = (tipoUtilidad === 'servicio' ? (configuracion?.utilidad_servicio ?? 0) : (configuracion?.utilidad_producto ?? 0)) / 100;
        ...
        const montoBase = subtotal + sobreprecioMonto;

        const denominador = 1 - comisionPorcentaje;
        const precioSistema = denominador > 0 ? montoBase / denominador : Infinity;
        const comisionVentaMonto = comisionPorcentaje * precioSistema;
