...
import { crearServicio } from '@/app/admin/_lib/servicio.actions';

interface Props {
    categoriaId?: string;
}

const FormCrearServicio = ({ categoriaId }: Props) => {
    ...
    const handleSubmit = async () => {
        ...
        const result = await crearServicio(nuevoServicio);
        if (result.success) {
    ...
    useEffect(() => {
        const calcularPrecioSistema = () => {
            ...
            const sobreprecio_monto = costo_total * (sobreprecio_porcentaje / 100);
            ...
            setSobreprecioMonto(sobreprecio_monto);
    ...
    }, [costo, gastosTemporales, configuracion?.comision_venta, configuracion?.sobreprecio, utilidadPorcentaje]);

    //! calculo de precio  publico
    useEffect(() => {
        const calcularPrecioPublico = () => {
            const precio_publico = parseFloat(precioPublico);
    ...
    }, [precioPublico, comisionDeVentaPorcentaje, sobreprecioMonto, costo, gasto, sobreprecioPorcentaje]);

    useEffect(() => {
        if (configuracion) {
            const utilidad = tipo === 'producto' ? configuracion.utilidad_producto : configuracion.utilidad_servicio;
            setUtilidadPorcentaje(utilidad);
        }
    }, [tipo, configuracion]);
    ...
    return (
        <div className='text-black p-5 mx-auto max-w-screen-sm'>
            ...
            <div className='grid grid-cols-2 items-center mb-5'>
                ...
                <div className='flex justify-end'>
                    <label className="flex items-center cursor-pointer">
                        ...
                        <span className="ml-3 text-zinc-500 text-xl ">
            ...
            <div className="">
                ...
                <div className='text-zinc-600 border-zinc-700 rounded-md mb-3 grid grid-cols-2 gap-2'>
                    {/* PRECIO SISTEMA */}
                    <div className="p-3 border border-zinc-600 rounded-md">
                        <label className="block text-gray-500 text-sm mb-1">
                            Precio del sistema
