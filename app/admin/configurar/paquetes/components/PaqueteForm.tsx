        ...
        <form onSubmit={handleSubmit(onSubmit)} className="max-w-full mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                ...
                <div className="lg:col-span-1 space-y-6">
                    ...
                    {analisisPresupuesto.serviciosPorCosto.length > 0 && (
                        <div className="p-3 rounded-lg border border-purple-600/50 bg-purple-950/20 space-y-2 text-[11px] text-purple-200/80">
                            <h4 className="text-sm font-semibold text-purple-100 mb-2 flex items-center justify-between">
                                <span>Análisis de Presupuesto</span>
                                <span className="text-[10px] text-purple-300/60">Top servicios</span>
                            </h4>
                            <div className="space-y-1.5">
                                {analisisPresupuesto.serviciosPorCosto.map((servicio, index) => {
                                    const porcentaje = analisisPresupuesto.totalPresupuesto > 0
                                        ? (servicio.costoTotalCompleto / analisisPresupuesto.totalPresupuesto) * 100
                                        : 0;
                                    return (
                                        <div key={servicio.id} className="flex items-center justify-between text-[10px]">
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-1">
                                                    <span className={`w-4 h-4 rounded-full text-[8px] flex items-center justify-center font-bold ${index === 0 ? 'bg-red-500/80 text-white' :
                                                        index === 1 ? 'bg-orange-500/80 text-white' :
                                                            index === 2 ? 'bg-yellow-500/80 text-black' :
                                                                'bg-purple-500/60 text-white'
                                                        }`}>
                                                        {index + 1}
                                                    </span>
                                                    <span className="truncate text-purple-100">{servicio.nombre}</span>
                                                    {servicio.cantidad > 1 && (
                                                        <span className="text-purple-300/60">x{servicio.cantidad}</span>
                                                    )}
                                                </div>
                                                <div className="w-full bg-purple-900/40 rounded-full h-1.5 mt-1">
                                                    <div
                                            ...
                                            <div className="text-right ml-2">
                                                <div className="font-medium text-purple-100">
                                                    {servicio.costoTotalCompleto.toLocaleString('es-MX', { style: 'currency', currency: 'MXN' })}
                                                </div>
                                                <div className="text-purple-300/70">{porcentaje.toFixed(1)}%</div>
                                            </div>
                                        </div>
                                    );
                                })}
                            </div>
                            <div className="pt-2 border-t border-purple-700/50 mt-2">
                                <div className="flex justify-between text-[10px]">
                                    <span>Total presupuesto base:</span>
                                    <span className="font-medium text-purple-100">
                                        {analisisPresupuesto.totalPresupuesto.toLocaleString('es-MX', { style: 'currency', currency: 'MXN' })}
                                    </span>
                                </div>
                                <div className="flex justify-between text-[10px] text-purple-300/70">
                                    <span>Top 5 representan:</span>
                                    <span>
                                        {analisisPresupuesto.serviciosPorCosto.reduce((sum, s) => {
                                            const pct = analisisPresupuesto.totalPresupuesto > 0
                                                ? (s.costoTotalCompleto / analisisPresupuesto.totalPresupuesto) * 100
                                                : 0;
                                            return sum + pct;
                                        }, 0).toFixed(1)}%
                                    </span>
                    ...
                    {/* Selects Condición y Método */}
                    {condiciones.length > 0 && (
                        <div>
                            <label className="block text-sm font-medium text-zinc-300 mb-1.5">Condición Comercial</label>
                            <select value={condicionId} onChange={e => setCondicionId(e.target.value)} className="flex h-10 w-full items-center rounded-md border border-zinc-700 bg-zinc-900 px-3 text-sm">
                                <option value="">-- Sin descuento --</option>
                                {condiciones.map(c => <option key={c.id} value={c.id}>{c.nombre} {c.descuento ? `(${c.descuento}%)` : ''}</option>)}
                            </select>
                        </div>
                    )}
                    {metodosPago.length > 0 && (
                        <div>
                            <label className="block text-sm font-medium text-zinc-300 mb-1.5">Método de Pago</label>
                            <select value={metodoPagoId} onChange={e => setMetodoPagoId(e.target.value)} className="flex h-10 w-full items-center rounded-md border border-zinc-700 bg-zinc-900 px-3 text-sm">
                                <option value="">-- No aplicar extras --</option>
                                {metodosFiltrados.map(m => <option key={m.id} value={m.id}>{m.metodo_pago}</option>)}
                            </select>
                        </div>
                    )}
