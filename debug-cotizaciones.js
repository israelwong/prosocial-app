...
async function debugCotizaciones(eventoId) {
    try {
        ...
        console.log(`   - Etapa: ${evento.EventoEtapa?.nombre || 'No definida'} (posición: ${evento.EventoEtapa?.posicion || 0})`);
        console.log('');
        ...
        console.log(`📋 Total de cotizaciones en el evento: ${todasLasCotizaciones.length}`);
        if (todasLasCotizaciones.length > 0) {
            ...
            todasLasCotizaciones.forEach((cot, index) => {
                ...
                console.log(`      - Visible cliente: ${cot.visible_cliente ? '✅' : '❌'}`);
                console.log(`      - Precio: $${cot.precio?.toLocaleString('es-MX') || 'No definido'}`);
                console.log(`      - Creada: ${cot.createdAt.toLocaleDateString('es-MX')}`);
                console.log('');
            });
        }

        // 3. Filtrar cotizaciones visibles al cliente
        const cotizacionesVisibles = todasLasCotizaciones.filter(cot => 
            cot.visible_cliente && ['pending', 'pendiente', 'aprobada', 'approved'].includes(cot.status)
        );

        console.log(`👁️  Cotizaciones visibles al cliente: ${cotizacionesVisibles.length}`);
        ...
        if (evento.eventoTipoId) {
            ...
            if (paquetes.length > 0) {
                paquetes.forEach((paq, index) => {
                    console.log(`   ${index + 1}. ${paq.nombre} - $${paq.precio?.toLocaleString('es-MX')}`);
                });
            }
        }
        ...
        if (cotizacionesVisibles.length === 0) {
            ...
            console.log(`   → redireccion_automatica a /evento/${eventoId}/cotizacion/${cotizacionesVisibles[0].id}`);
        } else {
            console.log(`   → mostrar_lista con ${cotizacionesVisibles.length} cotizaciones`);
        }
